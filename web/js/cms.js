  ...
  function () {
    ...
    _createClass(FileCollection, [{
      ...
      value: function getFileListUrl(type, config) {
        return 'web/' + (config.mode === 'GITHUB' ? getGithubUrl(type, config.github) : type);
      }
      /**
       * Get file URL.
       * @method
      ...
      value: function getFileUrl(file, mode, type) {
        return 'web/' + (mode === 'GITHUB' ? file['download_url'] : "".concat(type, "/").concat(getFilenameFromPath(file.getAttribute('href'))));
      }
      /**
       * Get file elements.
